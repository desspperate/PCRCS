...
static void pair_node_destruct_control(struct pair_node* node_)
{
    free(node_);
}
...
{
    free(node_->pair_.key);
    free(node_);
}

void pair_list_destruct_control(struct pair_node** head)
{
    ...
    while (*head != NULL) {
        ...
        *head = pair_node_next(*head);
        pair_node_destruct_control(prev);
    }   
}
