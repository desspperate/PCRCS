...
static void bucket_destruct_control(struct bucket* bucket_)
{
    pair_list_destruct_control(&bucket_->head);
    free(bucket_);
}
...
static void map_destruct_control(struct map *map_)
{
    for (int i = 0; i < map_->capacity; ++i) {
        bucket_destruct_control(*(map_->storage + i));
    }
